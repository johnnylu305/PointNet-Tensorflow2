...
import numpy as np
from tensorflow.keras import layers, Sequential, activations, initializers, Input, optimizers, backend
    ...
    def __init__(self, name, K):
        ...
        if 'Tnet_1' in name:
            net_para = [['{}_reshape1'.format(name), [-1, 3, 1]],
                        ['{}_conv2d_1'.format(name), [64, (1, 3), (1, 1), 'valid', 'relu', True, None]],
...
class bn_decay(optimizers.schedules.LearningRateSchedule):
    def __init__(self, momentum=0.5, decay_steps=6250, decay_rate=0.5, staircase=True, clip=0.99):
        ...
        self.clip = clip
        self.schedule = tf.keras.optimizers.schedules.ExponentialDecay(momentum,
                                                                       decay_steps=decay_steps,
                                                                       decay_rate=decay_rate,
                                                                       staircase=staircase)
    def __call__(self, step):
...
class conv2d(layers.Layer):
    def __init__(self, layer):
        ...
        if layer[1][6]:
            self.model.add(layers.Dropout(layer[1][6]))
...
class concate(layers.Layer):
    def __init__(self, layer):
        super(concate, self).__init__()
        # [batch, 1, 1, category]
        self.one_hot_label = layers.Reshape(target_shape=layer[1][0])
        self.max_out_exp = layers.Reshape(target_shape=[1, 1, -1])
        self.add_max_out = tf.keras.layers.Concatenate(axis=layer[1][1])
        # [batch, num_point, 1, category]
        self.expand = layers.Lambda(backend.tile, arguments={'n':layer[1][2]})
        # [batch, num_point, 1, category+layer_outs]
        self.add_layer_outs = tf.keras.layers.Concatenate(axis=layer[1][3])

    def call(self, label, layer_outs, max_out):
        x = self.one_hot_label(label)
        x = self.add_max_out([self.max_out_exp(max_out), x])
        x = self.expand(x)
        x = self.add_layer_outs([x, 
                                 layer_outs[0], 
                                 layer_outs[1], 
                                 layer_outs[2], 
                                 layer_outs[3], 
