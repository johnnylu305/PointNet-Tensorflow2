...
def test(model, x, cls_label, seg_label, name=None):
    ...
    for i in range(iteration):
        ...
        instance_accuracy = model.accuracy(pred, seg_label[start:end])
        acc.append(np.sum(instance_accuracy))
        #if args.vis:
...
if __name__=="__main__":
    if args.phase=='train':
        ...
        for i in range(start, args.epoch):
            ...
            accuracy = test(model, val_x, val_y, val_seg)
            # save
        ...
        if saver.restore(checkpoint):
            print("Load checkpoint succeeded")
        # test
        accuracy = test(model, test_x, test_y, test_seg)
        print(accuracy)
