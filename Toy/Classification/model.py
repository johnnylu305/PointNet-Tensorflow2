...
class Classification(layers.Layer):
    def __init__(self, name, lr):
        ...
        lr_schedule = lr_decay(lr,
                               ...
                               staircase=True,
                               clip=0.00001)
        self.bn_decay = bn_decay(momentum=0.9, 
                                 decay_steps=6250, 
                                 decay_rate=0.5, 
                                 staircase=True, 
                                 clip=0.99)
        ...
        self.net_para = [['{}_Tnet_1'.format(name), [3]],
                    ...
                    ['{}_dense_3'.format(name), [2, None, None, None]],
                    ['{}_sigmoid'.format(name), ['sigmoid']]]
        self.nets = []
        for layer in self.net_para:
            ...
            elif 'sigmoid' in layer[0]:
                self.nets.append(layers.Activation(activations.sigmoid))
    ...
    def loss(self, pred, label, matrix, alpha): 
        ...
        return tf.reduce_mean(losses.BinaryCrossentropy(from_logits=False)(label, pred))+alpha*l2_norm

    def accuracy(self, pred, label):
        pred = tf.where(pred>=0.5, 1, 0)
        correct_predictions = tf.reduce_all(tf.equal(pred, label), 1)
