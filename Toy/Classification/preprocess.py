...
def get_data(args):
    ...
    if args.phase=="train":
        ...
        for i, p in enumerate(paths):
            ...
            # normalization
            train_x[i] = (point_cloud_np-np.mean(point_cloud_np, axis=0))/np.std(point_cloud_np, axis=0)
        ...
        for i, p in enumerate(paths):
            ...
            idx = np.random.choice(point_cloud_np.shape[0], size=args.sampling, replace=False)
            point_cloud_np = point_cloud_np[idx]
            val_x[i] = (point_cloud_np-np.mean(point_cloud_np, axis=0))/np.std(point_cloud_np, axis=0)
            for color in np.unique(np.array(point_cloud.colors), axis=0):
        ...
        for i, p in enumerate(paths):
            ...
            idx = np.random.choice(point_cloud_np.shape[0], size=args.sampling, replace=False)
            point_cloud_np = point_cloud_np[idx]
            test_x[i] = (point_cloud_np-np.mean(point_cloud_np, axis=0))/np.std(point_cloud_np, axis=0)
            for color in np.unique(np.array(point_cloud.colors), axis=0):
