...
import datetime
import glob
from preprocess import get_data, shuffle, rotate_point_cloud, jitter_point_cloud, rotate_point_cloud_angle
...
parser.add_argument('--alpha', dest='alpha', type=float, default=0.00001, help='weight for regularization loss')
parser.add_argument('--batch_size', dest='batch_size', type=int, default=32, help='# point cloud in batch')
parser.add_argument('--epoch', dest='epoch', type=int, default=40, help='# epoch')
...
def train(model, x, y, val_x=None, val_y=None):
    ...
    x, y = shuffle(x, y)
    # augment
    x_ = jitter_point_cloud(rotate_point_cloud(x))
    dataset_size = len(x)
    ...
    for i in range(iteration):
        ...
        with tf.GradientTape() as tape:
            pred, matrix = model(x_[start:end], training=True)
...
def test(model, x, y, name=None):
    pred, matrix = model(x, training=False)
    if args.vis:
        model_dir = "{}_{}".format(args.type, TIME)
        visualize(x, y, pred, name, os.path.join(args.vis_dir, model_dir), 100)
    return model.accuracy(pred, y).numpy()
